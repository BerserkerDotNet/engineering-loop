...
After inspecting the app diff and codebase context, the coordinator may add one or more specialist
...
`COORDINATOR_SESSION_ID`, `RUN_ID`, `PHASE`, monotonic `SEQUENCE`, review workspace session ID,
source revision, merge base, `access_digest`, and `review_digest`. Children use the app's native
changes overview and diff, may read repository context, and never ask the user directly.
`review_digest` hashes role, model, prompt version, `ReviewWorkspace`, and access.

Prompts are capped at 16 KiB, envelopes at 64 KiB, a single finding at 4 KiB, and findings at
100 per role. Overflow blocks rather than truncates.

Read `../prompts/area-review.md` before launching reviewers. Require `REVIEW_COMPLETE` with
attested revisions/digests and either `[<Area>] <Text>` findings with changed-line citations or
explicit no violations. `BLOCKED` is the only other outcome.
All four baseline areas and every additional launched topic must complete.

## Reconciliation and exploration

Verify session, sequence, source revision, merge base, `access_digest`, and `review_digest`;
reject stale envelopes and recheck the review workspace. Present the pinned revision, short change summary, codebase
...
answers cross-area questions from the app diff and repository context, cannot mutate findings or
drafts, and routes new area claims to the owning reviewer. It is superseded on drift.
...
- Workspace revision or diff drift: discard affected reviews and refresh them at the new revision.
